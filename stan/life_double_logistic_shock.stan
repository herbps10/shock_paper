...
transformed parameters {
  ...
  matrix[C, T - 1] pos_shock = rep_matrix(0, C, T - 1);
  ...
  vector<lower=0>[n_shocks] truncated_pos_local_shrinkage = sqrt(square(pos_c_slab) * square(pos_local_shrinkage) ./ (square(pos_c_slab) + square(pos_global_shrinkage) * square(pos_local_shrinkage)));; 
  pos_shock = to_matrix(pos_shock_raw .* truncated_pos_local_shrinkage * pos_global_shrinkage, C, T - 1);
...
generated quantities {
  ...
  matrix[C, Tpred - 1] pos_shock2 = rep_matrix(0, C, Tpred - 1);
  ...
  pos_shock2[1:C, 1:(T - 1)] = pos_shock;
  
  for(t in T:(Tpred)) {
    for(c in 1:C) {
      pos_shock2[c, t - 1] = shock_rng(nu_local, pos_c_slab, pos_global_shrinkage);
    }
    
    vector[C] transition = rate_double_logistic(eta[, t - 1], Delta1, Delta2, Delta3, Delta4, k, z);
    for(c in 1:C) {
      real error = normal_rng(0, epsilon_scale);
      eta[c, t] = eta[c, t - 1] + transition[c] + error + pos_shock2[c, t - 1];
    }
