...
included_codes <- include_2010 |>
  filter(include_code %in% 1:2) |>
  pull(country_code)
...
datM <- e0M1 |>
  ...
  mutate(year = parse_integer(str_sub(period, 1, 4)), source = "WPP2024")
...
e0_differences |>
  ...
  labs(
    x = expression(paste("Difference in ", e[0], ": ", eta[ct] - eta[ct - 1])),
    y = "Count"
  )
...
fits <- expand_grid(
  ...
  filter(!(data_model == "normal" & shock == FALSE)) |>
  mutate(
    fit = pmap(
      list(
        transition,
