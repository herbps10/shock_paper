fits <- read_rds("results.rds") |>
  filter(cutoff == 2025, scale_global == 0.1)

fit_shock <- fits$fit[[2]]
fit_outlier <- fits$fit[[3]]

epsilon_scale <- tidybayes::spread_draws(
  fit_noshock$samples$draws("epsilon_scale"),
  epsilon_scale
) |>
  mutate(model = "No shocks") |>
  bind_rows(
    tidybayes::spread_draws(
      fit_shock$samples$draws("epsilon_scale"),
      epsilon_scale
    ) |>
      mutate(model = "Shocks")
  )
...
epsilon_scale |>
  ...
  geom_function(
    fun = function(x) invgamma::dinvgamma(x, 0.1, 0.1),
    aes(color = "Prior")
  ) +
  xlim(c(0, 2))
...
ci_width_comparison <- fit_outlier$posteriors$temporal |>
  filter(variable == "eta", year == 2100) |>
  mutate(ci_width_outlier = `90%` - `10%`) |>
  select(name, year, ci_width_outlier) |>
  left_join(
  ...
  left_join(
    fit_shock$posteriors$temporal |>
      filter(variable == "eta_shockfree", year == 2100) |>
      mutate(ci_width_shocks_shockfree = `90%` - `10%`) |>
      select(name, year, ci_width_shocks_shockfree)
  )

projection_comparison <- fit_outlier$posteriors$temporal |>
  filter(year == 2100, variable == "eta") |>
  mutate(median_outlier = `50%`) |>
  select(name, year, median_outlier) |>
  left_join(
    fit_shock$posteriors$temporal |>
      filter(year == 2100, variable == "eta") |>
...
(p1 + p2) +
  plot_annotation(
    "Male period life expectancy by country, 2100",
    tag_levels = "A"
  )
...
# Fit comparisons
#
eta <- fit_outlier$posteriors$temporal |>
  filter(variable == "eta") |>
  mutate(model = "No shock term, outliers excluded") |>
  bind_rows(
    fit_shock$posteriors$temporal |>
      filter(variable == "eta") |>
      mutate(model = "Regularized shocks")
  )

shocks <- fit_shock$posteriors$temporal |>
  filter(variable == "shock2")

transitions <- fit_outlier$posterior$transition_functions |>
  mutate(model = "No shock term, outliers excluded") |>
  bind_rows(
...
countries <- projection_comparison |>
  mutate(
    abs_diff = abs(median_outlier - median_shocks),
    diff = median_outlier - median_shocks
  ) |>
  arrange(abs_diff)
...
arranged_countries <- c(
  arrange(countries, abs_diff)$name[1:4],
  arrange(countries, -abs_diff)$name[1:4]
)

e0diff <- fit_shock$data |>
  group_by(name) |>
  mutate(e0diff = c(diff(e0), NA)) |>
  left_join(select(shocks, name, year, `50%`)) |>
  rename(shock_median = `50%`)
...
eta |>
  ...
  geom_ribbon(
    aes(ymin = `2.5%`, ymax = `97.5%`, fill = model),
    color = "transparent",
    alpha = 0.2
  ) +
...
transitions |>
  filter(name %in% country, .width == 0.8) |>
  ggplot(aes(x = x, y = transition_function_pred)) +
  #geom_lineribbon(aes(ymin = .lower, ymax = .upper, group = model)) +
  geom_point(
    data = filter(e0diff, name %in% country),
    aes(x = e0, y = e0diff, shape = abs(shock_median) > 1),
    alpha = 0.5
  ) +
  geom_line(aes(color = model)) +
  #geom_line(aes(color = model, y = .lower), lty = 2) +
  #geom_line(aes(color = model, y = .upper), lty = 2) +
  scale_fill_brewer() +
...
lower_shocks_countries <- projection_comparison |>
  mutate(
    abs_diff = abs(median_no_shocks - median_shocks),
    diff = median_no_shocks - median_shocks
  ) |>
  filter(diff > 1) |>
  arrange(diff) |>
  pull(name)
...
fit_shock$posteriors$transition_functions |>
  ...
  geom_line(
    aes(color = "No Shock"),
    data = filter(
      fit_noshock$posterior$transition_functions,
      name %in% areas,
      .width == 0.5
    )
  ) +
...
fits$fit[[1]]$posteriors$temporal |>
  ...
  geom_line(
    aes(color = "No shock"),
    data = fits$fit[[2]]$posteriors$temporal |>
      filter(name %in% areas, variable == "eta")
  ) +
  facet_wrap(~name)
...
fit_shock$posteriors$temporal |>
  ...
  geom_line(
    aes(color = "No shock"),
    data = fit_noshock$posteriors$temporal |>
      filter(name %in% areas, variable == "eta")
  ) +
  facet_wrap(~name)
...
p1 <- plot_mean_transition(fit_noshock) +
  ylim(c(0, 11)) +
  labs(x = expression(e0), y = expression(f[b])) +
  theme(legend.position = "none") +
  ggtitle(label = "No shocks")
p2 <- plot_mean_transition(fit_shock) +
  ylim(c(0, 11)) +
  labs(x = expression(e0), y = expression(f[b])) +
  theme(legend.position = "none") +
...
fit_noshock$posteriors$transition_function_mean |>
  mutate(model = "No shock") |>
  bind_rows(
    fit_shock$posteriors$transition_function_mean |> mutate(model = "Shock")
  ) |>
...
caux |>
  ...
  geom_density() +
  geom_function(aes(color = "Prior"), fun = function(x) {
    invgamma::dinvgamma(x, 0.5 * stan_data$slab_df, 0.5 * stan_data$slab_df)
  })
...
global_shrinkage <- fits |>
  filter(model == "shock2") |>
  mutate(
    global_shrinkage = map(fit, function(fit) {
...
countries <- largest_shocks(fit_shock) |>
  select(name, year, `2.5%`, `50%`, `97.5%`) |>
  pull(name) |>
  unique()
...
for (country in countries) {
  ...
  p2 <- plot_indicator(fit_shock, country) +
    ...
    labs(
      x = "Year",
      y = expression(e[0]),
      subtitle = expression(shocks ~ (tau[0] == 0.01))
    )
